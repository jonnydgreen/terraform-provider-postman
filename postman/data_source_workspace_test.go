...
func TestAccWorkspaceDataSource__basicWithDescription(t *testing.T) {
	resourceName := "postman_workspace.default"
	dataSourceName := "data.postman_workspace.default"
	workspaceName := acctest.RandomWithPrefix("tf-test")
	workspaceType := "personal"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Read testing
			{
				Config: testAccWorkspaceDataSource__basicWithDescription(context),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrPair(dataSourceName, "id", resourceName, "id"),
					resource.TestCheckResourceAttrPair(dataSourceName, "name", resourceName, "name"),
					resource.TestCheckResourceAttrPair(dataSourceName, "type", resourceName, "type"),
...
func testAccWorkspaceDataSource__basicWithDescription(context map[string]interface{}) string {
	return Nprintf(providerConfig+`
resource "postman_workspace" "default" {
  name = "%{name}"
  type = "%{type}"
